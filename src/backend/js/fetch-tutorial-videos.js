...
async function fetchVideos() {
  ...
  if (!categoryId) {
    videoIcons.innerHTML = `<p class="dark:text-white">Error: Category ID is missing.</p>`;
    return;
  }

  try {
    let response = await fetch(
      `backend/php/fetch-tutorial-videos.php?uuid=${categoryId}`
    );
    ...
    if (result.status === "success" && Array.isArray(result.data)) {
      ...
      videoIcons.innerHTML = `<p class="dark:text-white">${result.message}</p>`;
    }
  } catch (error) {
    console.error("Fetch error:", error);
    videoIcons.innerHTML = `<p class="dark:text-white">Failed to load videos.</p>`;
  }
}
